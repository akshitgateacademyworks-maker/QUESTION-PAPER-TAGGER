...
def build_tagged_docx(questions: list[Question], out_path: str) -> None:
    """Write `questions` out as a fully self-contained, standard-compliant
    tagged .docx at `out_path` (no external template file dependency)."""
...
# --------------------------------------------------------------------------
# Full, standalone OOXML package (mirrors what real Word produces: content
# types, package + document relationships, docProps, styles, fonts,
# settings, web settings). No binary template asset is required, which
# avoids deployment issues where an external file fails to ship/resolve.
# --------------------------------------------------------------------------
...
_CORE_PROPS = (
    ...
    '<dc:title>Tagged Questions</dc:title>'
    '<dc:creator>MCQ Docx Tagger</dc:creator>'
    '<cp:lastModifiedBy>MCQ Docx Tagger</cp:lastModifiedBy>'
...
_APP_PROPS = (
    ...
    '<Application>MCQ Docx Tagger</Application>'
    '</Properties>'
)
...
_STYLES_XML = (
    ...
    '</w:rPr></w:rPrDefault>'
    '<w:pPrDefault><w:pPr><w:spacing w:after="20"/></w:pPr></w:pPrDefault>'
    '</w:docDefaults>'
